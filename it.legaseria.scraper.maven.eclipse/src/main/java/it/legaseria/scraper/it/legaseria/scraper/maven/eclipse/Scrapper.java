...
public class Scrapper {
	...
	public void giornate(int daQuesta, int finoQuesta) {
		...
		for (int page = daQuesta; page <= finoQuesta; page++) {
			String url = "http://www.legaseriea.it/it/serie-a/calendario-e-risultati/2020-21/UNICO/UNI/"
					+ String.valueOf(page);
	...
	private void extractData(Elements repositories, ArrayList<MatchDay> daysOfPlay, int round) {
		...
		for (Element repo : repositories) {
			String matchDetails = repo.getElementsByClass("datipartita").text();
			

			
			//Match date and time
			String[] dateAndTime = matchDetails.substring(0, 16).replaceAll("[a-zA-Z]", "").trim().split(" ");
			...
			// Match Teams
			String teams = repo.getElementsByClass("nomeSquadra").text();
			...
			if (matchDetails.contains("Vedi")) {                             // Some matches weren't shown on TV and the scrapped info is different
				channel = channel.substring(0, 4).trim();
			}
			...
			daysOfPlay.get(daysOfPlay.size() - 1).addMatch(match);
